...
public class Board {
    ...
    private String[][][]genNewBoard(int[] size, int bombCount, int[] startCords){
        ...
        for (int i = 0; i < bombCount; i++){
            ...
            if (isStartCoordinate(startCords,x,y) || Objects.equals(newBoard[x][y][0], "B")){
                i--;
            ...
            for (int j  = -1; j<2;j++){
                try {
                    ...
                    if(((i % 2 == 0 && j % 2 != 0)|| (i % 2 != 0 && j % 2 == 0)) || (i/j == -1 || i/j == 1)){
                        if(!Objects.equals(this.Board[cords[0] + i][cords[1] + j][1], "p")){
                            this.Board[cords[0]+i][cords[1]+j][1] = "p";
                            if (Objects.equals(this.Board[cords[0] + i][cords[1] + j][0], "0")){
    ...
    public boolean winCheck(){
        ...
        for(int i = 0; i < this.Board.length;i++){
            for(int j =0; j < this.Board.length;j++){
                if(!Objects.equals(Board[i][j][1], "f") && Objects.equals(Board[i][j][0], "B")) {
                    return false;
                }
            }
        }
    ...
    public boolean lossCheck(int[] cords){
        ...
        // return  (this.Board[cords[0]][cords[1]][0]).equals("B");
